...
function insertBook(transaction, book){
    ...
    return executeSqlInTransaction(transaction, 'INSERT INTO books(title, author, isbn) OUTPUT INSERTED.id VALUES(@title, @author, @isbn)',
        { 'title': book.title, 'author': book.author, 'isbn': book.isbn}).then(
            result => {
...
function insertCopy(transaction, bookId){
    return executeSqlInTransaction(transaction, 'INSERT INTO copies(bookId) OUTPUT INSERTED.id, INSERTED.bookId VALUES(@bookId)',
        { 'bookId': bookId } ).then(
            result => {
